...
const theme: GreetingTheme = {
  ...
  description: (
    <>
      ...
      <span className="font-bold text-primary">新規友だち</span> のみ稼働します。
    </>
  ),
...
export default function GreetingsNewFriendPage() {
  const { currentChannelId } = useAuth();
  const { data, isLoading, error } = useResource(
    currentChannelId ? `greeting-form:${currentChannelId}:new_friend` : null,
    async () => {
      const [greeting, channels, options, tags, chatStatuses] = await Promise.all([
        fetchRawGreeting("new_friend"),
        fetchFormChannels(),
        fetchBuilderOptions(),
        fetchTags(),
        fetchChatStatuses(),
  ...
  return (
    <GreetingForm
      type="new_friend"
      theme={theme}
      submitUrl="/greetings/new_friend"
